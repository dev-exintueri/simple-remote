...
describe("viewer and relay", () => {
	it("viewer_to_offline_host_404", async () => {
		...
		expect((await request(`/v1/viewer/987654321?kind=new`, freshIp())).status).toBe(404);
	});

	it("relays_both_ways", async () => {
		const { host, viewer, n } = await pair();
		host.send({ t: "relay", n, data: "00ff10" });
		expect(await viewer.next()).toEqual({ t: "relay", data: "00ff10" });
		viewer.send({ t: "relay", data: "abcdef" });
		expect(await host.next()).toEqual({ t: "relay", n, data: "abcdef" });
	});

	it("second_viewer_409", async () => {
		const { id } = await pair();
		expect((await request(`/v1/viewer/${id}?kind=new`, freshIp())).status).toBe(409);
	});

	it("kick_closes_viewer_4001", async () => {
		const { host, viewer, id, n } = await pair();
		host.send({ t: "kick", n });
		expect(await viewer.closeCode()).toBe(4001);
		expect(await host.silentFor(200)).toBe(true);
		// the slot is free again
		const next = await connect(`/v1/viewer/${id}?kind=new`, freshIp());
	...
	it("relay_from_kicked_viewer_never_reaches_host", async () => {
		...
		const next = await connect(`/v1/viewer/${id}?kind=new`, freshIp());
		expect(await next.next()).toEqual({ t: "joined" });
		expect(await host.next()).toEqual({ t: "viewer_joined", n: n + 1, kind: "new" });
		next.send({ t: "relay", data: "b1" });
		expect(await host.next()).toEqual({ t: "relay", n: n + 1, data: "b1" });
		expect(await host.silentFor(200)).toBe(true);
	});
	...
	it("viewer_close_sends_viewer_left", async () => {
		...
		expect(await host.next()).toEqual({ t: "viewer_left", n });
	});

	it("oversize_message_closes_4013", async () => {
		const { host } = await pair();
		host.send({ t: "relay", n: 1, data: "a".repeat(65_536) });
	...
	it("host relay without viewer is ignored", async () => {
		...
		peer.send({ t: "relay", n: 1, data: "00" });
		peer.send({ t: "kick", n: 1 });
	...
	it("viewer_rate_limited_429", async () => {
		const ip = freshIp();
		for (let i = 0; i < 30; i++) {
			expect((await request(`/v1/viewer/111111111?kind=new`, ip)).status).toBe(404);
		}
		expect((await request(`/v1/viewer/111111111?kind=new`, ip)).status).toBe(429);
	});
});
